...
func TestPrepareHomeAuthSnapshotPersistsPreparedMetadata(t *testing.T) {
	store := &requestPrepareStore{}
	executor := &requestPrepareExecutor{}
	manager := NewManager(store, nil, nil)
	auth := &Auth{
		ID:       "home-prepare-persist",
		Provider: "antigravity",
		Status:   StatusActive,
		Metadata: map[string]any{"access_token": "same-token"},
	}
	if _, errRegister := manager.Register(WithSkipPersist(context.Background()), auth); errRegister != nil {
		t.Fatalf("register auth: %v", errRegister)
	}
	...
	if got := testStringValue(prepared.Metadata["project_id"]); got != "prepared-project" {
		t.Fatalf("prepared project_id = %q, want prepared-project", got)
	}
	if got := store.saveCount.Load(); got < 1 {
		t.Fatalf("save count = %d, want at least 1", got)
	}
	...
	if got := testStringValue(current.Metadata["access_token"]); got != "same-token" {
		t.Fatalf("manager access token = %q, want same-token", got)
	}
}

func TestPrepareHomeAuthSnapshotDoesNotPersistConflictingLocalCredential(t *testing.T) {
	store := &requestPrepareStore{}
	executor := &requestPrepareExecutor{}
	manager := NewManager(store, nil, nil)
	localAuth := &Auth{
	...
	prepared, errPrepare := manager.prepareHomeAuthSnapshot(context.Background(), executor, homeAuth)
	if errPrepare != nil {
		t.Fatalf("prepareHomeAuthSnapshot() error = %v", errPrepare)
	}
	if got := testStringValue(prepared.Metadata["access_token"]); got != "home-token" {
		t.Fatalf("prepared access token = %q, want Home token", got)
	}
	if got := testStringValue(prepared.Metadata["project_id"]); got != "prepared-project" {
		t.Fatalf("prepared project_id = %q, want prepared-project", got)
	}
	...
	if got := store.saveCount.Load(); got != 0 {
		t.Fatalf("save count = %d, want 0 when Home snapshot would clobber a different local credential", got)
	}
}
