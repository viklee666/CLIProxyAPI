...
func emptyStreamResult(result *cliproxyexecutor.StreamResult) *cliproxyexecutor.StreamResult {
	if result != nil {
		return result
	}
	return &cliproxyexecutor.StreamResult{}
...
func retryStreamBootstrap(ctx context.Context, executor ProviderExecutor, auth *Auth, execReq cliproxyexecutor.Request, execOpts cliproxyexecutor.Options) (*cliproxyexecutor.StreamResult, []cliproxyexecutor.StreamChunk, bool, error) {
	...
	errStream = markUpstreamExecutionAttemptFromContext(ctx, errStream)
	if errStream != nil {
		return streamResult, nil, false, errStream
	}
	buffered, closed, bootstrapErr := readStreamBootstrap(ctx, streamResult.Chunks)
	bootstrapErr = markUpstreamExecutionAttemptFromContext(ctx, bootstrapErr)
	return streamResult, buffered, closed, bootstrapErr
}
...
func (m *Manager) wrapStreamResult(ctx context.Context, auth *Auth, provider, resultModel, routeModel string, headers http.Header, buffered []cliproxyexecutor.StreamChunk, remaining <-chan cliproxyexecutor.StreamChunk, aliasResult OAuthModelAliasResult, ephemeralResult bool, opts cliproxyexecutor.Options, onDone func()) *cliproxyexecutor.StreamResult {
...
func (m *Manager) executeStreamWithModelPool(ctx context.Context, executor ProviderExecutor, auth *Auth, provider string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, routeModel, executionModel string, execModels []string, pooled bool, aliasResult OAuthModelAliasResult, routing *apiKeyModelRoutingSnapshot, allowRetry bool, ephemeralResult bool, onDone func()) (*cliproxyexecutor.StreamResult, error) {
	...
	for idx, execModel := range execModels {
		...
		if errStream != nil {
			...
			// Agent Identity assertions are repaired per request before falling back to OAuth refresh.
		...
		if bootstrapErr != nil {
			...
			if recovered, okRecover, errRecover := m.tryRecoverRequestAuth(ctx, executor, auth, bootstrapErr, didRecoverRequestAuth); okRecover {
				...
				if errRecover != nil {
					...
					publishSelectedAuthMetadata(execOpts.Metadata, auth)
					ctx = newUpstreamAttemptContext(ctx)
					retryStream, retryBuffered, retryClosed, retryErr := retryStreamBootstrap(ctx, executor, auth, execReq, execOpts)
					if retryErr != nil {
						if errCtx := ctx.Err(); errCtx != nil {
							if retryStream != nil {
						...
						streamResult = emptyStreamResult(retryStream)
					} else {
						streamResult = retryStream
						buffered = retryBuffered
						closed = retryClosed
						bootstrapErr = nil
					}
				}
		...
		wrapped := m.wrapStreamResult(ctx, auth.Clone(), provider, resultModel, routeModel, streamResult.Headers, buffered, remaining, attemptAliasResult, ephemeralResult, execOpts, onDone)
		handedOff = true
		return wrapped, nil
	}
	if lastErr == nil {
