...
func (m *Manager) executeMixedOnce(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, maxRetryCredentials int, retryRound int, defaultRequestRetry int) (cliproxyexecutor.Response, error) {
	...
	attempt := newAdaptiveAttemptTracker()
...
func (m *Manager) executeStreamMixedOnce(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, maxRetryCredentials int, homeRetryLimit *int, retryRound int, defaultRequestRetry int) (*cliproxyexecutor.StreamResult, error) {
	...
	for {
		...
		streamResult, errStream := m.executeStreamWithModelPool(execCtx, executor, auth, provider, execReq, execOpts, routeModel, streamExecutionModel, models, pooled, aliasResult, routing, !homeMode || selection != nil, selection != nil, attempt.handoff())
		if errStream != nil {
			if hasUpstreamExecutionAttempt(errStream) {
...
// prepareHomeRequestAuth prepares a dispatch auth without reading local auth
// state as the prepare target. Successful repairs are persisted via Update
// using the prepared snapshot's ID.
...
func (m *Manager) prepareHomeAuthSnapshot(ctx context.Context, executor ProviderExecutor, auth *Auth) (*Auth, error) {
	...
	prepare := func() (*Auth, error) {
		...
		return m.persistPreparedHomeAuth(ctx, updated)
	}
...
func (m *Manager) persistPreparedHomeAuth(ctx context.Context, updated *Auth) (*Auth, error) {
	if m == nil || updated == nil {
		return updated, nil
	}
	...
	// Persist the prepared snapshot's ID via Update, but never replace a
	// different local credential that happens to share that ID.
	if existing, ok := m.GetByID(id); ok && existing != nil {
		existingToken := AccessTokenSHA256(existing)
		preparedToken := AccessTokenSHA256(updated)
		if existingToken != "" && preparedToken != "" && existingToken != preparedToken {
			return updated, nil
		}
	}
...
func (m *Manager) HttpRequest(ctx context.Context, auth *Auth, req *http.Request) (*http.Response, error) {
	...
	for {
		...
		recoveryErr := errDo
		// Only inspect a 401 body when this executor can actually repair request-scoped
		// auth; otherwise the probe would consume a body the caller still owns.
		if recoveryErr == nil && !recoveryTried && executorSupportsRequestAuthRecovery(exec, auth) {
