...
func normalizedRoutingRuntimeState(cfg *config.Config) routingRuntimeState {
	...
	state.adaptive = cfg.Routing.Adaptive
...
func routingRuntimeStateEqual(prev, next routingRuntimeState) bool {
	if prev.strategy != next.strategy || prev.sessionAffinity != next.sessionAffinity {
		return false
	}
	if prev.strategy == "adaptive" && prev.adaptive != next.adaptive {
		return false
	}
...
func unregisterAdaptiveUsagePlugin() {
	plugin := usage.UnregisterNamedPlugin(adaptiveUsagePluginName)
	if stoppable, ok := plugin.(interface{ Stop() }); ok {
		stoppable.Stop()
	}
}
...
func newRoutingSelector(state routingRuntimeState) coreauth.Selector {
	...
	case "adaptive":
		...
		// RegisterNamed replaces any previous adaptive plugin in place.
		adaptive := coreauth.NewAdaptiveSelector(state.adaptive)
