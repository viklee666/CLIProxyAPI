...
import type { AuthFilesListQuery } from '@/services/api';
...
export function AuthFilesPage() {
  ...
  const authFilesQuery = useMemo<AuthFilesListQuery | undefined>(() => {
    if (!serverPaginationEnabled) return undefined;
    ...
    if (normalizedFilter !== 'all') query.provider = normalizedFilter;
    if (debouncedSearch.trim()) query.search = debouncedSearch.trim();
    if (disabledOnly) query.disabled = true;
    if (healthyOnly) query.healthy = true;
    else if (problemOnly) query.problem = true;
    ...
    switch (sortMode) {
      ...
      case 'note-desc':
        query.sort = 'note';
        query.order = sortMode === 'note-desc' ? 'desc' : 'asc';
        break;
      case 'priority-asc':
      case 'priority-desc':
        query.sort = 'priority';
        query.order = sortMode === 'priority-desc' ? 'desc' : 'asc';
        break;
      default:
        query.sort = 'provider';
        query.order = 'asc';
  ...
  const {
    ...
    batchPatchFields,
    batchDelete,
  } = useAuthFilesData(authFilesQuery);
  ...
  const typeCounts = useMemo(() => {
    if (serverPaged) {
      return { all: serverTotal, ...providerFacets };
    }
    ...
    return counts;
  }, [filesMatchingStatusFilters, providerFacets, serverPaged, serverTotal]);
  ...
  const paginationTotal = serverPaged ? serverTotal : sorted.length;
  const totalPages = Math.max(1, Math.ceil(paginationTotal / pageSize));
  const currentPage = Math.min(page, totalPages);
  const start = (currentPage - 1) * pageSize;
  const pageItems = useMemo(
    () => (serverPaged ? sorted : sorted.slice(start, start + pageSize)),
    [pageSize, serverPaged, sorted, start]
  );
      ...
      <section className={styles.authFilesShell}>
        ...
        <div className={styles.filterSection}>
          ...
          <div className={styles.filterContent}>
            ...
            {!loading && paginationTotal > pageSize && (
