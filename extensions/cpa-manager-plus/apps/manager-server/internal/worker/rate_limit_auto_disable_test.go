...
func TestQuotaAutoDisableCandidateAcceptsProviderNeutralQuotaRecovery(t *testing.T) {
	now := time.Unix(1_700_000_000, 0)
	event := usage.Event{
		EventHash:              "evt-provider-quota",
		Failed:                 true,
		FailStatusCode:         http.StatusPaymentRequired,
		AuthFileSnapshot:       "claude-auth.json",
		AuthIndex:              "auth-claude-1",
		AccountSnapshot:        "user@example.com",
		Provider:               "claude",
		HeaderQuotaRecoverAtMS: now.Add(30 * time.Minute).UnixMilli(),
	...
	candidate, ok := quotaAutoDisableCandidateFromEvent(event, "http://cpa", "key", now)
	if !ok {
		t.Fatal("provider-neutral quota candidate not detected")
	}
	if candidate.Provider != "claude" || candidate.ResetAt.UnixMilli() != event.HeaderQuotaRecoverAtMS {
