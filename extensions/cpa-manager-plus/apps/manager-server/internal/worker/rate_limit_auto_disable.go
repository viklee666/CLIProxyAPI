...
func (w *RateLimitAutoDisableWorker) recoverCooldown(ctx context.Context, baseURL string, managementKey string, item store.QuotaCooldown, now time.Time) {
	if item.Owner != model.QuotaCooldownOwnerUsage429 &&
		item.Owner != model.QuotaCooldownOwnerXAIFreeUsage &&
		item.Owner != model.QuotaCooldownOwnerCodexInspection {
...
func quotaAutoDisableCandidateFromEvent(event usage.Event, baseURL string, managementKey string, now time.Time) (quotaAutoDisableCandidate, bool) {
	...
	if !ok {
		...
		resetAt, ok = knownQuotaResetTimeFromEvent(event, now)
		if !ok {
			return quotaAutoDisableCandidate{}, false
		}
	}
...
// knownQuotaResetTimeFromEvent accepts provider-neutral quota failures only
// when response metadata contains an explicit future quota recovery timestamp.
// Generic Retry-After and transient 5xx responses remain runtime cooldowns in
// CLIProxyAPI and are intentionally not persisted as disabled credentials.
func knownQuotaResetTimeFromEvent(event usage.Event, now time.Time) (time.Time, bool) {
	if !event.Failed {
	...
	signal := strings.ToLower(strings.TrimSpace(event.HeaderErrorKind + " " + event.HeaderErrorCode))
	if !strings.Contains(signal, "quota") && !strings.Contains(signal, "rate") && !strings.Contains(signal, "limit") {
		return time.Time{}, false
	}
	recoverAtMS := event.HeaderQuotaRecoverAtMS
	if event.ResponseMetadata != nil && event.ResponseMetadata.Quota != nil && event.ResponseMetadata.Quota.RecoverAtMS > recoverAtMS {
		recoverAtMS = event.ResponseMetadata.Quota.RecoverAtMS
	}
	if recoverAtMS <= 0 {
		return time.Time{}, false
	}
	resetAt := time.UnixMilli(recoverAtMS)
