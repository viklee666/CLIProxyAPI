...
func (r *authSnapshotResolver) lookup(ctx context.Context, cfg RuntimeConfig, authIndices map[string]struct{}) map[string]authSnapshot {
	...
	snapshots, err := r.fetch(ctx, baseURL, managementKey, authIndices)
	if err != nil {
		r.mu.Lock()
...
func (r *authSnapshotResolver) fetch(ctx context.Context, baseURL string, managementKey string, authIndices map[string]struct{}) (map[string]authSnapshot, error) {
	endpoint, err := authFilesEndpoint(baseURL, authIndices)
	if err != nil {
		return nil, err
	}
...
func authFilesEndpoint(baseURL string, authIndices map[string]struct{}) (string, error) {
	...
	query := parsed.Query()
	query.Set("view", "snapshot")
	for authIndex := range authIndices {
		if authIndex = strings.TrimSpace(authIndex); authIndex != "" {
			query.Add("auth_index", authIndex)
		}
	}
