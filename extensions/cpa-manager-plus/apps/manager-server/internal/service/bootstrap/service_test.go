...
func TestEnsureAdminCredentialSynchronizesIntegratedKey(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "usage.sqlite")
	...
	oldCredential, err := security.NewAdminCredential("old-manager-key", "legacy")
	if err != nil {
	...
	created, generated, err := ensureAdminCredential(context.Background(), config.Config{
		AdminKey:       "shared-management-key",
		IntegratedMode: true,
	}, st)
	if err != nil {
		t.Fatalf("ensure integrated credential: %v", err)
	}
	if created || generated != "" {
		t.Fatalf("created=%v generated=%q", created, generated)
	}

	credential, ok, err := st.LoadAdminCredential(context.Background())
	if err != nil || !ok {
		t.Fatalf("load synchronized credential ok=%v err=%v", ok, err)
	}
	if !security.VerifyAdminKey(credential, "shared-management-key") {
		t.Fatal("shared management key does not verify")
	}
	if security.VerifyAdminKey(credential, "old-manager-key") {
		t.Fatal("old manager key still verifies")
	}
}
