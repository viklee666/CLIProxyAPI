...
func ensureAdminCredential(ctx context.Context, cfg config.Config, st *store.Store) (bool, string, error) {
	existing, ok, err := st.LoadAdminCredential(ctx)
	if err != nil {
		return false, "", err
	}
	if ok {
		if !cfg.IntegratedMode || cfg.AdminKey == "" || security.VerifyAdminKey(existing, cfg.AdminKey) {
			return false, "", nil
		}
		credential, errCredential := security.NewAdminCredential(cfg.AdminKey, "integrated")
		if errCredential != nil {
			return false, "", errCredential
		}
		credential.CreatedAtMS = existing.CreatedAtMS
		credential.RotatedAtMS = time.Now().UnixMilli()
		if errSave := st.SaveAdminCredential(ctx, credential); errSave != nil {
	...
	source := "env"
	if cfg.IntegratedMode && adminKey != "" {
		source = "integrated"
	}
	if adminKey == "" {
