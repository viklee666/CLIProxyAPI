...
// xaiChatBaseURL returns the base URL for xAI HTTP chat/stream requests.
...
// still honored.
// Media, websocket, and compact transports intentionally do not use this helper:
// cli-chat-proxy does not implement image/video endpoints, /responses/compact
// (404), or websocket upgrades (405).
func xaiChatBaseURL(auth *cliproxyauth.Auth) string {
	_, baseURL := xaiCreds(auth)
	return xaiauth.SelectChatBaseURL(baseURL, xaiUsingAPI(auth))
}

// xaiMediaBaseURL returns the base URL for xAI image/video requests.
// Media stays on the official API (or an explicit custom gateway) even when
// using_api is false. The CLI chat-proxy does not serve those endpoints.
func xaiMediaBaseURL(auth *cliproxyauth.Auth) string {
	_, baseURL := xaiCreds(auth)
	return xaiauth.SelectMediaBaseURL(baseURL, xaiUsingAPI(auth))
}
