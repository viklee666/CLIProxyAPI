...
func classifyCodexStatusError(statusCode int, body []byte) []byte {
	...
	if code == "invalid_task_id" {
		// The upstream message embeds the task id; never surface it downstream.
...
func codexStatusErrorClassification(statusCode int, body []byte) (code string, errType string, ok bool) {
	...
	switch {
	// An invalid Agent Identity task is recoverable per request, so it must not be
	// collapsed into the generic auth_unavailable classification below.
