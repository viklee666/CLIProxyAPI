...
// applyCodexHeadersFromSources fails closed for Agent Identity credentials: without a
// freshly generated assertion the request must not be sent with a stale bearer token.
func applyCodexHeadersFromSources(r *http.Request, auth *cliproxyauth.Auth, token string, stream bool, cfg *config.Config, ginHeaders http.Header) error {
	...
	if isAgentIdentityAuth(auth) {
		assertion, errAssertion := generateAgentAssertion(auth)
		if errAssertion != nil {
			return fmt.Errorf("codex executor: generate agent assertion: %w", errAssertion)
		}
	...
	if !isAPIKey {
		...
		// An empty account_id must not clobber the Agent Identity fallback.
		if accountID == "" {
