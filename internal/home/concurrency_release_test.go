...
func TestReleaseFlusherForceFlushSkipsCancelledContext(t *testing.T) {
	flusher := newReleaseFlusher(time.Hour, time.Hour, func(context.Context, ConcurrencyReleaseFrame) error { return nil })
	stale, cancelStale := context.WithCancel(context.Background())
	cancelStale()
	flusher.forceFlush(stale)
	select {
	case <-flusher.force:
		t.Fatal("cancelled Flush occupied the force slot")
	default:
	}
}

type forceFlushMark struct{ name string }

func TestReleaseFlusherForceFlushReplacesParkedContext(t *testing.T) {
	flusher := newReleaseFlusher(time.Hour, time.Hour, func(context.Context, ConcurrencyReleaseFrame) error { return nil })
	first := context.WithValue(context.Background(), forceFlushMark{name: "first"}, true)
	flusher.forceFlush(first)
	second := context.WithValue(context.Background(), forceFlushMark{name: "shutdown"}, true)
	flusher.forceFlush(second)
	select {
	case got := <-flusher.force:
		if got != second {
			t.Fatal("force slot kept the older Flush context")
		}
	default:
		t.Fatal("force slot empty after replace")
	}
}
