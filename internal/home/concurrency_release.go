...
func (f *releaseFlusher) forceFlush(ctx context.Context) {
	...
	// A cancelled Flush must not occupy the 1-slot force channel, or a later
	// shutdown Flush cannot enqueue and the dirty groups stay unacked.
	if ctx != nil {
		select {
		case <-ctx.Done():
			return
	...
	for {
		...
		case f.force <- ctx:
			return
		case <-f.force:
			// Drop a parked waiter so the latest Flush can replace it.
		}
	}
}
