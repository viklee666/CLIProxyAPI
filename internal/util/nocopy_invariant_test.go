...
var reviewedInPlaceByteWrites = map[string]reviewedInPlaceByteWrite{
	...
	"internal/runtime/executor/agent_identity.go":                                            {2, "copies a SHA-512 digest and X25519 public key into local fixed-size arrays; no request payload is rewritten"},
	...
	"extensions/cpa-manager-plus/apps/manager-server/internal/service/modelprice/service.go": {1, "copies already-decoded candidate structs into a newly allocated slice; no request payload is rewritten"},
}
