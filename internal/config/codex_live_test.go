...
func TestParseConfigBytesRejectsInvalidLiveMediaRelay(t *testing.T) {
...
`))
	if errParse == nil {
		t.Fatal("ParseConfigBytes() error = nil, want invalid live-media-relay rejection")
	}
}

func TestParseConfigBytesAllowsInvalidLiveMediaRelayWhenDisabled(t *testing.T) {
...
`))
	...
	if cfg.Codex.LiveMediaRelay.Enabled {
		t.Fatal("LiveMediaRelay.Enabled = true, want disabled optional-mode skip")
	}
}
