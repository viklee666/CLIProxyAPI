...
func TestParseConfigBytesAppliesCredentialInFlightDefaultsForOmittedSection(t *testing.T) {
	...
	if cfg.CredentialInFlight.SnapshotInterval != DefaultCredentialInFlightConfig().SnapshotInterval {
		t.Fatalf("SnapshotInterval = %q, want default", cfg.CredentialInFlight.SnapshotInterval)
	}
	if cfg.CredentialInFlight.MaxDetails != DefaultInFlightMaxDetails {
		t.Fatalf("MaxDetails = %d, want %d", cfg.CredentialInFlight.MaxDetails, DefaultInFlightMaxDetails)
	}
}

func TestParseConfigBytesPreservesExplicitZeroCredentialInFlightMaxDetails(t *testing.T) {
	cfg, errParse := ParseConfigBytes([]byte("host: 127.0.0.1\ncredential-in-flight:\n  max-details: 0\n"))
	if errParse != nil {
		t.Fatalf("ParseConfigBytes() error = %v", errParse)
	}
	...
	if cfg.CredentialInFlight.MaxDetails != 0 {
		t.Fatalf("MaxDetails = %d, want 0", cfg.CredentialInFlight.MaxDetails)
	}
	if cfg.CredentialInFlight.SnapshotInterval != DefaultCredentialInFlightConfig().SnapshotInterval {
		t.Fatalf("SnapshotInterval = %q, want default", cfg.CredentialInFlight.SnapshotInterval)
	}
...
func assertOrderedJSONFields(t *testing.T, structType reflect.Type, want []jsonField) {
	t.Helper()
	exported := make([]reflect.StructField, 0, structType.NumField())
	...
	if len(exported) != len(want) {
		t.Fatalf("%s exported field count = %d, want %d", structType.Name(), len(exported), len(want))
	}
	for index, expected := range want {
		field := exported[index]
