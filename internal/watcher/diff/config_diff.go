...
func BuildConfigChangeDetails(oldCfg, newCfg *config.Config) []string {
	...
	if strings.TrimSpace(oldCfg.Routing.SessionAffinityTTL) != strings.TrimSpace(newCfg.Routing.SessionAffinityTTL) {
		changes = append(changes, fmt.Sprintf("routing.session-affinity-ttl: %s -> %s", displayOptionalValue(oldCfg.Routing.SessionAffinityTTL), displayOptionalValue(newCfg.Routing.SessionAffinityTTL)))
	}
