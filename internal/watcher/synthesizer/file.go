...
func synthesizeFileAuths(ctx *SynthesisContext, fullPath string, data []byte) ([]*coreauth.Auth, error) {
	...
	if ctx.PluginAuthParser != nil {
		...
		if errParse == nil && handled {
			...
			fileAuthKind := resolveFileAuthKind(metadata)
			...
			for index, auth := range auths {
				...
				ApplyAuthExcludedModelsMeta(auth, cfg, perAccountExcluded, fileAuthKind)
	...
	ApplyAuthExcludedModelsMeta(a, cfg, perAccountExcluded, resolveFileAuthKind(metadata))
