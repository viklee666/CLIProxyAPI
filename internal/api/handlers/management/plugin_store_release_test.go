...
func TestListPluginStoreSkipsUninstalledReleases(t *testing.T) {
	...
	for range 2 {
		...
		byID := make(map[string]pluginstore.Plugin, len(registry.Plugins))
		for _, plugin := range registry.Plugins {
			byID[plugin.ID] = plugin
		}
		for _, entry := range body.Plugins {
			plugin, ok := byID[entry.ID]
			if !ok || entry.Version != plugin.Version || entry.Installed || entry.UpdateAvailable {
