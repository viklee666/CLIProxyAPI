...
func (s *Server) unifiedModelsHandler(openaiHandler *openai.OpenAIAPIHandler, claudeHandler *claude.ClaudeCodeAPIHandler) gin.HandlerFunc {
	return func(c *gin.Context) {
		if grokbuild.IsGrokShellUserAgent(c.GetHeader("User-Agent")) {
			// Tenant client-access keys must never reach the global Home or registry catalog.
			if openaiHandler != nil && openaiHandler.IsTenantRequest(c) {
				s.handleTenantGrokModels(c, openaiHandler)
				return
			}
		...
		// Tenant client-access keys must never reach the global Home catalog.
...
func grokModelsFromCatalogMaps(catalog []map[string]any) []grokbuild.ModelInfo {
	models := make([]grokbuild.ModelInfo, 0, len(catalog))
	for _, entry := range catalog {
		...
		model := grokbuild.ModelInfo{ID: id}
		if displayName, ok := entry["display_name"].(string); ok {
			model.DisplayName = displayName
		} else if displayName, ok := entry["displayName"].(string); ok {
			model.DisplayName = displayName
		}
		model.ContextLength = grokIntFromAny(entry["context_length"])
		if model.ContextLength <= 0 {
			model.ContextLength = grokIntFromAny(entry["max_context_length"])
		}
		model.ReasoningLevels = grokReasoningLevelsFromCatalogEntry(entry)
		models = append(models, model)
	}
	return models
}

func grokIntFromAny(value any) int {
	switch n := value.(type) {
	case int:
		return n
	case int32:
...
func grokReasoningLevelsFromCatalogEntry(entry map[string]any) []string {
	if entry == nil {
		return nil
	}
	if levels := grokStringSliceFromAny(entry["reasoning_levels"]); len(levels) > 0 {
		return levels
	}
	switch thinking := entry["thinking"].(type) {
	...
	case registry.ThinkingSupport:
		return append([]string(nil), thinking.Levels...)
	case map[string]any:
		return grokStringSliceFromAny(thinking["levels"])
	default:
		return grokStringSliceFromAny(thinking)
	}
}

func grokStringSliceFromAny(value any) []string {
