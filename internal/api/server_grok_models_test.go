...
func TestGrokModelsFromCatalogMaps(t *testing.T) {
	models := grokModelsFromCatalogMaps([]map[string]any{
		{
			"id":             "tenant-model",
		...
		},
		{
			...
			"reasoning_levels":   []string{"medium"},
		},
		nil,
		{"object": "model"},
	})
	if len(models) != 2 {
		t.Fatalf("catalog model count = %d, want 2", len(models))
	}
	if models[0].ID != "tenant-model" || models[0].DisplayName != "Tenant Model" || models[0].ContextLength != 128000 {
		t.Fatalf("tenant catalog adapter = %#v", models[0])
	}
	if len(models[0].ReasoningLevels) != 2 || models[0].ReasoningLevels[0] != "low" || models[0].ReasoningLevels[1] != "high" {
		t.Fatalf("tenant reasoning levels = %#v", models[0].ReasoningLevels)
	}
	if models[1].ID != "alias-model" || models[1].DisplayName != "Alias Model" || models[1].ContextLength != 64000 {
		t.Fatalf("alias catalog adapter = %#v", models[1])
	}
	if len(models[1].ReasoningLevels) != 1 || models[1].ReasoningLevels[0] != "medium" {
		t.Fatalf("alias reasoning levels = %#v", models[1].ReasoningLevels)
	}
}
