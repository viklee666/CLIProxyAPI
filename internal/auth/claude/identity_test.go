...
func TestReadMetadataBoolUsesMetadataLock(t *testing.T) {
	var unset *map[string]any
	if ReadMetadataBool(unset, "skip_account_profile") {
		t.Fatal("nil metadata pointer should return false")
	}
	metadata := map[string]any{"skip_account_profile": true, "is_setup_token": false, "scopes": "user:inference"}
	if !ReadMetadataBool(&metadata, "skip_account_profile") {
		t.Fatal("skip_account_profile = false, want true")
	}
	if ReadMetadataBool(&metadata, "is_setup_token") {
		t.Fatal("is_setup_token = true, want false")
	}
	if ReadMetadataBool(&metadata, "missing") {
		t.Fatal("missing bool = true, want false")
	}
	value, ok := ReadMetadata(&metadata, "scopes")
	if !ok || value != "user:inference" {
		t.Fatalf("ReadMetadata(scopes) = (%v, %t), want user:inference", value, ok)
	}
}
