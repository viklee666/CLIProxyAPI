...
// refreshTokenRequest is the refresh-token body. Field order is significant:
// it mirrors the native Claude Code 2.1.220 traffic to
// platform.claude.com/v1/oauth/token, matching the login struct's grant_type-first
// layout. A map would be re-sorted alphabetically by encoding/json.
