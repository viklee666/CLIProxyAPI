...
func TestRefreshTokenRequestKeyOrderMatchesLoginExchange(t *testing.T) {
	loginBody, errLogin := json.Marshal(authorizationCodeExchangeRequest{
	...
	if errLogin != nil {
		t.Fatalf("marshal login body: %v", errLogin)
	}
	refreshBody, errRefresh := json.Marshal(refreshTokenRequest{
	...
	if errRefresh != nil {
		t.Fatalf("marshal refresh body: %v", errRefresh)
	}
	if !strings.HasPrefix(string(loginBody), `{"grant_type":`) {
		t.Fatalf("login body = %q, want grant_type as the first key", loginBody)
	}
	if !strings.HasPrefix(string(refreshBody), `{"grant_type":`) {
		t.Fatalf("refresh body = %q, want grant_type as the first key like login", refreshBody)
	}
	wantRefresh := `{"grant_type":"refresh_token","refresh_token":"placeholder-refresh","client_id":"` + ClientID + `","scope":"` + ClaudeOAuthScope + `"}`
	if got := string(refreshBody); got != wantRefresh {
		t.Fatalf("refresh body = %q, want stable field order %q", got, wantRefresh)
	}
}
